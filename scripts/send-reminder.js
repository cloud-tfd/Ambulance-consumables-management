 ...
 *
 * 功能：
 *  1. 從 Firebase 讀取最新耗材資料
 *  2. 計算已過期 / 即將到期的耗材
 *  3. 透過 EmailJS REST API 發送提醒郵件
 */

const FIREBASE_URL    = process.env.FIREBASE_URL;       // Firebase RTDB base URL
...
// ── 設定：到期提醒天數閾值 ────────────────────────────────
const ALERT_DAYS = [1, 7, 14, 30];   // 30天內到期的都會列入
const MAX_DAYS   = Math.max(...ALERT_DAYS);

// ── 工具：HTTP fetch（Node 18+ 內建）────────────────────────
...
async function main() {
  ...
  console.log(`\n📡 從 Firebase 讀取資料: ${base}/ems_inventory_data.json`);
